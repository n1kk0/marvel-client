...
class HeroDescriptionTabView extends StatelessWidget {
  ...
  Widget _marvelLinkButton(String label, String url, BuildContext context) {
    return url != null ? RaisedButton(
      ...
      child: Row(
        children: <Widget>[
          Text(label, style: TextStyle(color: Theme.of(context).primaryTextTheme.bodyText1.color, fontSize: 16, fontWeight: FontWeight.bold)),
          Padding(padding: EdgeInsets.all(5)),
          Icon(Icons.open_in_new, color: Theme.of(context).primaryTextTheme.bodyText1.color, size: 16),
        ],
      ),
      onPressed: () async {
        if (kIsWeb) {
